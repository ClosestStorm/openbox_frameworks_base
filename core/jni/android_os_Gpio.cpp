...
extern int go_update_package(const char *path);
...
static int readGpio_native(JNIEnv *env, jobject clazz, jstring path)
{
	...
	else 
    {
        ...
        if(fd < 0){
            LOGE("fail in open file %s", chars);
        ...
        if(end == valueStr){
            LOGE("Fail in convert string %s to number.", valueStr);
            env->ReleaseStringUTFChars(path, chars);
            close(fd);
            return -1;
        }
...
static int writeGpio_native(JNIEnv *env, jobject clazz, jstring path, jstring value)
{
	...
	else 
    {
        ...
        fd = open(chars, O_RDWR);
        if(fd < 0){
            LOGD("fail in open file %s", chars);
            env->ReleaseStringUTFChars(path, chars);
