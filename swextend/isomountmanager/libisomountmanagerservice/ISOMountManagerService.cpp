...
namespace android {
	...
	void ISOMountManagerService::instantiate() {
	    defaultServiceManager()->addService(
	            String16("allwinner.isomountmanager"), new ISOMountManagerService());
	}
	
	ISOMountManagerService::ISOMountManagerService()
	{
//	    LOGD("ISOMountManagerService created");
	...
	ISOMountManagerService::~ISOMountManagerService()
	{
//	    LOGD("ISOMountManagerService destroyed");
	    if(mInfoArray != NULL){
	    	delete[] mInfoArray;
	    	mInfoArray = NULL;
	    }
	...
	void ISOMountManagerService::addMountInfo(const char *mountPath, const char *isoPath, const char *devPath){
		...
		if(mInfoArray[mCursor] == NULL){
			LOGE("ISOMountManagerService::addMountInfo()..ERROR!");
			return;
		}
	...
	void ISOMountManagerService::deleteMountInfo(const char *mountPath){
		...
		LOGE("ISOMountManagerService::deleteMountInfo()...ERROR!");
	}
...
//		if(DEBUG)
//			LOGD("ISOMountManagerService::isoMount(): mountPath = %s, isoPath = %s", mountPath, isoPath);
...
//		if(DEBUG)
//			LOGD("ISOMountManagerService::isoUmount(): mountPath = %s", mountPath);
...
//		if(DEBUG)
//			LOGD("ISOMountManagerService::umountAll()..");
...
//		if(DEBUG)
//			LOGD("ISOMountManagerService::getMountInfoList()..length = %d", length);
...
//		if(DEBUG)
//			LOGD("ISOMountManagerService::getIsoPath()..mountPath = %s", mountPath);
...
//		if(DEBUG)
//			LOGD("ISOMountManagerService::getMountPoints()..isoPath = %s, length = %d", isoPath, length);
