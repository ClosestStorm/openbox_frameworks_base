...
public:
	...
	bool createFile(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::createFile()  path = %s",path);
		Parcel data, reply;
	...
	bool deleteFile(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::deleteFile()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	bool exists(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::exists()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	long long getTotalSpace(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::getTotalSpace()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	long long getUsableSpace(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::getUsabelSpace()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
    ...
    {
    	if(DEBUG)
    		LOGV("ISecureFileService::isDirectory()  path = %s",path);
        Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	bool isFile(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::isFile()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	long long length(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::length()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	int getCount(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::getCount()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	int	list(const char *path, SecureItemName *itemArray, int count){
		if(DEBUG)
			LOGV("ISecureFileService::list() path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
    ...
    {
    	if(DEBUG)
			LOGV("ISecureFileService::mkDir()  path = %s",path);
        Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	unsigned long lastModified(const char *path){
		if(DEBUG)
			LOGV("ISecureFileService::lastModified()  path = %s",path);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	bool renameTo(const char *oldPath, const char *newPath){
		if(DEBUG)
			LOGV("ISecureFileService::renameTo()  oldPath = %s  newPath = %s",oldPath,newPath);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
	...
	bool setLastModified(const char *path, unsigned long time){
		if(DEBUG)
			LOGV("ISecueFileService::setLastModified()  path = %s  time = %ld",path,time);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeCString(path);
	...
	bool writeToFile(const char *srcPath, const char *desPath, bool append){
		if(DEBUG)
			LOGV("ISecureFileService::writeToFile()  srcPath = %s  desPath = %s  append = %d",srcPath,desPath,append);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
	...
	bool writeInData(int8_t *dat, int count, const char *desPath, bool append){
		if(DEBUG)
			LOGV("ISecureFileService::writeInData()  desPath = %s  append = %d  count = %d",desPath,append,count);
		Parcel data, reply;
        data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
        data.writeInt32(count);
		data.writeCString(desPath);
	...
	bool read(int8_t *dat, int count, const char *srcPath){
		if(DEBUG)
			LOGV("ISecureFileService::read()  srcPath = %s  count = %d",srcPath,count);
		Parcel data, reply;
		data.writeInterfaceToken(ISecureFileService::getInterfaceDescriptor());
		data.writeInt32(count);
		data.writeCString(srcPath);
...
IMPLEMENT_META_INTERFACE(SecureFileService, "com.allwinner.ISecureFileService");
