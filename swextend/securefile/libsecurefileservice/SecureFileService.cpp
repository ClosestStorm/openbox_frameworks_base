...
void SecureFileService::instantiate() {
    defaultServiceManager()->addService(
            String16("allwinner.securefile"), new SecureFileService());
}

SecureFileService::SecureFileService()
{
    LOGD("SecureFileService created");
}

SecureFileService::~SecureFileService()
{
    LOGD("SecureFileService destroyed");
}

bool SecureFileService::deleteFile(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::Delete: path = %s", path);
...
long long SecureFileService::length(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::length(): path = %s", path);
...
unsigned long SecureFileService::lastModified(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::lastModified(): path = %s", path);
	...
	LOGD("SecureFileService::lastModified(): length = %ld", l);
    return l;
}
...
{
	if(DEBUG)
    	LOGD("SecureFileService::isDirectory(): path = %s", path);
...
bool SecureFileService::isFile(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::isFile(): path = %s", path);
...
bool SecureFileService::setLastModified(const char *path, unsigned long ms) {
	if(DEBUG)
		LOGD("SecureFileService::setLastModified(): path = %s time = %ld", path, ms);
...
long long SecureFileService::getFreeSpace(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::getFreeSpace(): path = %s", path);
...
long long SecureFileService::getTotalSpace(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::getTotalSpace(): path = %s", path);
...
long long SecureFileService::getUsableSpace(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::getUsableSpace(): path = %s", path);
...
static bool readDirectory(const char *path, DirEntries& entries) {
	if(DEBUG)
		LOGD("SecureFileService::readDirectory(): path = %s", path);
...
int SecureFileService::list(const char *path, SecureItemName *itemArray, int count) {
	if(DEBUG)
		LOGD("SecureFileService::list(): path = %s", path);
...
int SecureFileService::getCount(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::getCount(): path = %s", path);
...
bool SecureFileService::mkDir(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::mkDir(): path = %s", path);
...
bool SecureFileService::createFile(const char *path) {
	if(DEBUG)
		LOGD("SecureFileService::createNewFile(): path = %s", path);
...
bool SecureFileService::renameTo(const char * oldPath, const char * newPath) {
	if(DEBUG)
		LOGD("SecureFileService::renameTo(): oldPath = %s newPath = %s", oldPath, newPath);
...
bool SecureFileService::writeToFile(const char * srcPath,const char * desPath,bool append){
	if(DEBUG)
		LOGD("SecureFileService::writeToFile(): srcPath = %s desPath = %s", srcPath, desPath);

	if(!doAccess(srcPath,F_OK | R_OK) || !doAccess(desPath,F_OK | W_OK)){
		LOGE("file access error");
		return false;
	}	
	...
	if(desFp == NULL){
		LOGE("cannot open file %s to write",desPath);
		fclose(srcFp);
		return false;
	}
...
bool SecureFileService::writeInData(int8_t * data,int count,const char * desPath,bool append){
	if(DEBUG)
		LOGD("SecureFileService::writeInData(): path = %s", desPath);
...
{
	if(DEBUG)
		LOGD("SecureFileService::read(): srcPath = %s  count = %d ", srcPath,count);
