...
status_t CedarXPlayer::setParameter(int key, const Parcel &request)
{
    ...
    switch(key)
    {
        case PARAM_KEY_DISABLE_ENCRYPTFILE:
        {
            mExtendMember->encrypt_enable = 0;
            LOGV("## setParameter: encrypt_enable = [%d]",mExtendMember->encrypt_enable);
            ret = OK;
            break;
        }
        case PARAM_KEY_ENABLE_ENCRYPTFILE:
        {
            mExtendMember->encrypt_enable = 1;
...
status_t CedarXPlayer::prepareAsync() {
	...
	if (1 == mExtendMember->encrypt_enable)
	{
    ...
    {
        mExtendMember->encrypt_enable = 0;
        mExtendMember->encrypt_file_format = 0;
