#define LOG_TAG "commandDispatcher-JNI"
...
static void nativeOpenVmouse(JNIEnv *env, jobject clazz){
	if (fd == NULL){
		...
		if (fd < 0)
		{	
			LOGE(" could not open dev/vmouse ");
		}
	}
}
...
int register_android_server_NativeDispather(JNIEnv* env)
{	
	int res = jniRegisterNativeMethods(env, "com/android/server/googleremote/CommandDispatcher",
