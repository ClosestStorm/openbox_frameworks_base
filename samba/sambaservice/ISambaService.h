...
class ISambaService: public IInterface
{
public:
    DECLARE_META_INTERFACE(SambaService);

	virtual int vbser_mount(char *sourcedir, char *target,
			char *filesystemtype, unsigned int flags, 
			char *options) = 0;
	virtual int vbser_umount(char *mountpoint) = 0;
};

// ----------------------------------------------------------------------------

class BnSambaService: public BnInterface<ISambaService>
{
public:
