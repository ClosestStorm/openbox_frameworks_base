...
int smbc_init(smbc_get_auth_data_fn fn, int debug)
{
	...
	if (!lp_load("/etc/smb.conf", True, False, False)) {
...
//	LOGD("parsed path: fname='%s' server='%s' share='%s' "
...
//
	...
	if (server[0] == (char)0) {
		...
		status = name_resolve_bcast(MSBROWSE, 1, &ip_list, &count);
		if (!status) {
			if (!ip_list) SAFE_FREE(ip_list);
			if (!find_master_ip(workgroup, &server_addr)) {
				...
				errno = ENOMEM;
				LOGD("finding \01\02__MSBROWSE__\02 is failed\n");
				return NULL;
			}

			ip_list = (struct in_addr *)malloc(sizeof(server_addr));
			if (ip_list == NULL) {
				if (smbc_file_table[slot]) {
					SAFE_FREE(smbc_file_table[slot]->fname);
					SAFE_FREE(smbc_file_table[slot]);
				...
				errno = ENOMEM;
				LOGD("ip_list is NULL \n");
				return NULL;
			} else {
				memcpy(ip_list, &server_addr, sizeof(server_addr));
			}
			count =1;
		}
		...
		for (i = 0; i < count && i < max_lmb_count; i++) {
			...
			cli = get_ipc_connect_master_ip(wg_ptr, &u_info, &ip_list[i]);
			...
			pstrcpy(server, cli->desthost);

			cli_shutdown(cli);

			if (workgroup[0] == (char)0 || server[0] == (char)0) {
				if (smbc_file_table[slot]) {
					SAFE_FREE(smbc_file_table[slot]->fname);
					SAFE_FREE(smbc_file_table[slot]);
				}
				errno = ENOMEM;
				LOGD("workgroup or server is NULL \n");
				return NULL;
			}
			...
			if (!cli_NetServerEnum(&srv->cli, workgroup,
						SV_TYPE_DOMAIN_ENUM,
						list_fn, (void *)smbc_file_table[slot])) {
				continue;
			}
			break;
		}
		...
		if (share[0] == (char)0) {
			...
			if (!srv &&
					!is_ipaddress(server) &&  /* Not an IP addr so check next */
				 ...
				 * Get the backup list ...
				 */
				if (!name_status_find(wgroup, 0, 0, rem_ip, buserver)) {
				...
				if (!cli_NetServerEnum(&srv->cli, server, SV_TYPE_ALL, list_fn,
			...
			}else if(srv || (resolve_name(server, &rem_ip, 0x20))) {
...
struct in_addr *smbc_serverip(char *server)
{
	struct smbc_file *fe;
	struct in_addr ipaddr, *ip_list = NULL, *bcast = NULL;
	static int ServerFD = -1;
	//char *server;
	//struct cli_state clistatus;
	int j, count, flags, ret;


	if(server){
		//if(resolve_name(server, &ipaddr, 0x1d) ||   /* Found LMB */
		//		resolve_name(server, &ipaddr, 0x1b) || /* Found DMB */
		if(resolve_name(server, &ipaddr, 0x20) || resolve_name(server, &ipaddr, 0x00)){
			return (&ipaddr);  /* Success, ret bytes of data ... */
		}

		ServerFD = open_socket_in( SOCK_DGRAM, 0, 0,
		...
		for (j=iface_count() - 1; !ip_list && j >= 0; j--) {
			bcast = iface_n_bcast(j);
			ip_list = name_query(ServerFD,server , 0,
							1, 1, *bcast, &count, &flags);
		}

		if(!ip_list) return NULL;
